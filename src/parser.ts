...
export const app = async (inputPath: string, outputPath: string) => {
...
const buildExcel = (outputPath: string): Promise<void> => {
    return new Promise(async (resolve, reject) => {
        ...
        for await (const inputData of data) {
            ...
            inputData.lessons.forEach((lesson: { name: string }) => {
                if (
                    lessonsWithUniqueName.findIndex(
                        (el) => el.name === lesson.name
                    ) === -1
                ) {
            ...
            for await (const el of uniqueLessons) {
                ...
                const groups = groupsWithThatLesson.map(
                    (lesson) => lesson.group
                )
                ...
                for await (const group of groups) {
                    ...
                    const hoursByFinal = groupWithThatLesson.details.filter(
                        (el) => el.title.includes("экзамен")
                    )?.[0]?.hours
                    ...
                    const checkHours = groupWithThatLesson.details.filter(
                        (el) => el.title.includes("проверка к")
                    )?.[0]?.hours
            ...
            for (let i = 0; i < 12; i++) {
                ...
                ws.cell(offset + 5 + inputData.lessons.length, i + 3)
                    .formula(
                        `IF(SUM(${letterInPosition}${
                            offset + 5
                        }:${letterInPosition}${
                            offset + 5 + inputData.lessons.length - 1
                        }) = 0, "", SUM(${letterInPosition}${
                            offset + 5
                        }:${letterInPosition}${
                            offset + 5 + inputData.lessons.length - 1
                        }))`
